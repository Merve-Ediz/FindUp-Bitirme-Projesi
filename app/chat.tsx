  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const data = snapshot.docs
        ...
        .filter((msg: any) => !msg.deletedBy?.[currentUserId]); // Benim sildiğim mesajları gösterme
      setMessages(data);
      setTimeout(
          ...
          onPress: async () => {
            ...
            for (const messageId of selectedMessages) {
              ...
              if (msgSnap.exists()) {
                ...
                // Eğer tüm katılımcılar silmişse gerçekten sil
                const chatSnap = await getDoc(
                  doc(db, "chats", params.chatId as string),
                );
                const participants: string[] = chatSnap.exists()
                  ? chatSnap.data().participants
                  : [];
                const allDeleted = participants.every(
                  (p: string) => deletedBy[p],
                );

                if (allDeleted) {
                  await deleteDoc(msgRef);
                } else {
                  await updateDoc(msgRef, { deletedBy });
                }
              }
            }
            setSelectionMode(false);
